...
/* Perlin noise (from reactbits.dev Waves) */
class Grad {
...
/**
 * Theme-aware animated wave-line background (canvas).
 * lineColor defaults to the current --line-soft token; pass a CSS color to override.
 */
export default function Waves({
  ...
  useEffect(() => {
    ...
    // Resolve theme color from CSS variable if none provided.
    const resolveColor = () => {
      if (lineColor) return lineColor
      const v = getComputedStyle(document.documentElement).getPropertyValue('--line-soft').trim()
      return v || 'rgba(0,0,0,.12)'
    }
    ...
    const movePoints = (time) => {
      ...
      const factor = l * mouse.vs * 0.00065
      ...
      lines.forEach((pts) => pts.forEach((p) => {
        ...
        if (dist < l) {
          ...
          p.cursor.vx += cosA * f * factor
          p.cursor.vy += sinA * f * factor
        }
    ...
    const onResize = () => { colorRef.current = resolveColor(); setSize(); setLines() }
    ...
    const onTouchMove = (e) => { const t = e.touches[0]; updateMouse(t.clientX, t.clientY) }

    setSize(); setLines()
    if (reduce) {
      drawLines() // static frame only
    } else {
      frameRef.current = requestAnimationFrame(tick)
    ...
    const themeObs = new MutationObserver(() => { colorRef.current = resolveColor() })
  ...
  return (
    <div ref={containerRef} className={`waves ${className}`} style={style} aria-hidden="true">
